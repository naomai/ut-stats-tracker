...
Public Class GSMasterServerConnection
    ...
    Private Sub receivedAbout(packet As Hashtable, ByRef destinationPacket As UTQueryPacket)
        ...
        With My.Application.Info
            aboutText = "// UTTracker Master Server Module; // 2014 Namonaki14; URL: http://amaki.no-ip.eu/uttracker/master/; // Contact: tm.dvtb at gmail.com; // Diagnostic info beyond this point"
            destinationPacket.Add("about", aboutText)
            destinationPacket.AddIndexed("diag", "AppAssemblyInfo: title=""" & My.Application.Info.Title & """, name=""" & .AssemblyName & "-" & .Version.Major & "." & .Version.Minor & "." & .Version.Build & """")
            destinationPacket.AddIndexed("diag", "FrameworkVer: " & System.Environment.Version.ToString)
            destinationPacket.AddIndexed("diag", "MasterTickCount: " & masterServer.tickCount)
            destinationPacket.AddIndexed("diag", "GSListSize: " & masterServer.gameSpyKeys.Count)
            destinationPacket.AddIndexed("diag", "Listener: " & masterServer.listener.ToString)
            destinationPacket.AddIndexed("diag", "IServerListProvider: " & masterServer.appServerList.GetType.FullName)
            destinationPacket.AddIndexed("diag", "Async: " & (Not IsNothing(masterServer.asyncThread)))
            Dim aboutBridge = masterServer.appServerList.getAboutInfo()
            For Each key In aboutBridge.Keys
                destinationPacket.AddIndexed("diag", key & ": " & aboutBridge(key))
            Next
        End With
        destinationPacket.setReadyToSend()
    End Sub
