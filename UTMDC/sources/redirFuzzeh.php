...
namespace UTMDC\RedirectFuzzeh{
	use \DOMDocument;
	...
	function prefetchForUrl($url,$fname,$recursive=false){
		$urx="http://ut.fuzzeh.com/redirect/";
		$localDbFile=__DIR__ . "/../redir_fuzzeh.txt";
	...
	function prefetchArray($url,$fname){
		...
		$localHtFile=__DIR__ . "/../redir_fuzzeh.html";
	...
	function findPackageUrl($pak){
		$fx=file(__DIR__ . "/../redir_fuzzeh.txt",FILE_IGNORE_NEW_LINES|FILE_SKIP_EMPTY_LINES);
		foreach($fx as $fa){
			$pname=strtok($fa,"\\");
			$purl=strtok("\r");
