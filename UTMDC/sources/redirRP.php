...
namespace UTMDC\RedirectRPClan{
	use \DOMDocument;
	...
	function prefetchForUrl($url,$fname,$recursive=false){
		$urx="http://rp.site.nfoservers.com/redirect/";
		$localDbFile=__DIR__ . "/../redir_rpclan.txt";
	...
	function prefetchArray($url,$fname){
		...
		$localHtFile=__DIR__ . "/../redir_rpclan.html";
	...
	function findPackageUrl($pak){
		$fx=file(__DIR__ . "/../redir_rpclan.txt",FILE_IGNORE_NEW_LINES|FILE_SKIP_EMPTY_LINES);
		foreach($fx as $fa){
			$pname=strtok($fa,"\\");
			$purl=strtok("\r");
