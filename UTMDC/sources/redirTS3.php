...
namespace UTMDC\RedirectTS3{
	...
	function prefetchForUrl($url,$fname,$recursive=false){
		//$urx="http://ts3.anewstart.fr/uz/";
		$urx="http://ts3.overmindserver.net/uz/";
		$localDbFile=__DIR__ . "/../redir_ts3.txt";
	...
	function prefetchArray($url,$fname){
		//echo "PRE: $url $fname\r\n";
		$localHtFile=__DIR__ . "/../redir_ts3.html";
		if(file_exists($localHtFile)){
			$ht=file_get_contents($localHtFile);
		}else{
			$ht=spoof_req($url, null, false, true);
			if(strlen($ht)<50){
				$ht=file_get_contents($localHtFile);
			}else{
	...
	function findPackageUrl($pak){
		$fx=file(__DIR__ . "/../redir_ts3.txt",FILE_IGNORE_NEW_LINES|FILE_SKIP_EMPTY_LINES);
		foreach($fx as $fa){
			$pname=strtok($fa,"\\");
			$purl=strtok("\r");
	...
	prefetch();
	//echo findPackageUrl("TBMedievalDeco");
}	
?>