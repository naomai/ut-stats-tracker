...
namespace UTMDC\RedirectUM{
	...
	function prefetchForUrl($url,$fname,$recursive=false){
		$urx="http://unreal-mayhem.com/umredirecting/";
		$localDbFile=__DIR__ . "/../redir_unrealmayhem.txt";
	...
	function prefetchArray($url,$fname){
		...
		$localHtFile=__DIR__ . "/../redir_unrealmayhem.html";
		...
		$maxz=array();
		$matc=preg_match_all('#alt="\[   \]"></td><td><a href="([^"]*)">([^<]*)</a></td><td align="right">#',$ht,$matz);
		if($matc){
			for($i=0; $i<$matc; $i++){
				$pname=htmlspecialchars_decode(trim($matz[1][$i]));
	...
	function findPackageUrl($pak){
		$fx=file(__DIR__ . "/../redir_unrealmayhem.txt",FILE_IGNORE_NEW_LINES|FILE_SKIP_EMPTY_LINES);
		foreach($fx as $fa){
			$pname=strtok($fa,"\\");
			$purl=strtok("\r");
