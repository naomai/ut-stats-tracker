...
namespace UTMDC\RedirectURH{
	...
	function prefetchForUrl($url,$fname,$recursive=false){
		$urx="http://unrealhorsemen.com/MADMODUT99_Redirect/";
		$localDbFile=__DIR__ . "/../redir_unrealhorsemen.txt";
	...
	function prefetchArray($url,$fname){
		...
		$localHtFile=__DIR__ . "/../redir_unrealhorsemen.html";
	...
	function findPackageUrl($pak){
		$fx=file(__DIR__ . "/../redir_unrealhorsemen.txt",FILE_IGNORE_NEW_LINES|FILE_SKIP_EMPTY_LINES);
		foreach($fx as $fa){
			$pname=strtok($fa,"\\");
			$purl=strtok("\r");
