...
namespace UTMDC\RedirectVMElite{
	use \DOMDocument;
	...
	function prefetchForUrl($url,$fname,$recursive=false){
		$urx="http://vmelite.site.nfoservers.com/vmuznoleach/";
		$localDbFile=__DIR__ . "/../redir_vmuznoleach.txt";
	...
	function prefetchArray($url,$fname){
		...
		$localHtFile=__DIR__ . "/../redir_vmuznoleach.html";
	...
	function findPackageUrl($pak){
		$fx=file(__DIR__ . "/../redir_vmuznoleach.txt",FILE_IGNORE_NEW_LINES|FILE_SKIP_EMPTY_LINES);
		foreach($fx as $fa){
			$pname=strtok($fa,"\\");
			$purl=strtok("\r");
