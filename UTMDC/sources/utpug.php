...
namespace UTMDC\UTPug{
	...
	function prefetchForUrl($url,$fname,$recursive=false){
		$urx="http://files.utpug.com/index.php?dir=maps/$url";
		$localDbFile=__DIR__ . "/../utpug.txt";
	...
	function prefetchArray($url,$fname){
		...
		$localHtFile=__DIR__ . "/../utpug.html";
		...
		$maxz=array();
		$matc=preg_match_all('/		<div><a href="([^"]*)" class="[bw]"><img src="[^"]*" alt="[^"]*" \/><strong>([^<]*)<\/strong>/',$ht,$matz);
		if($matc){
			for($i=0; $i<$matc; $i++){
				$pname=trim($matz[2][$i]);
	...
	function findMapUrl($mapname){
		$fx=file(__DIR__ . "/../utpug.txt",FILE_IGNORE_NEW_LINES|FILE_SKIP_EMPTY_LINES);
		foreach($fx as $fa){
			$pname=strtok($fa,"\\");
			$purl=strtok("\r");
